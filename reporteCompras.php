...
if(isset($_POST["export"]) && isset($_POST["desde"])!="" && isset($_POST["hasta"])!="")
{
	...
	for($count = 0; $count < $last_page; $count++)
	{
		...
		$table_columns = array("Fecha","Comprobante","Proveedores", "Cuit", "Importe bruto" , "Desc" , "Neto Gravado", "No grav exentos", "Ret IB", "IVA", "Total Facturado" );
		$column = 0;
    ...
    where c.fecha BETWEEN CAST('".$_POST["desde"]."' AS DATE) AND CAST('".$_POST["hasta"]."' AS DATE) AND c.justificante='F' AND c.tipo='C'
		";
    ...
    //recorrer array con los datos filtrados por campaña para descargar
		...
		{
      $retib=((2.50*$sub_row["totalcomprado"])/100);
      $retib = bcdiv($retib, '1', 2);
      $iva=((21*$sub_row["totalcomprado"])/100);
      $iva = bcdiv($iva, '1', 2);
      $facturado= $sub_row["totalcomprado"] + $iva + $retib;
      ...
      $totalRet=$totalRet+$retib;
      $totalRet = bcdiv($totalRet, '1', 2);
			$excel_row++;
		}
      ...
      $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $totalRet);
      $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, $totalIva);
      $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $totalFac);
			// FALTA DESC, EXENTOS, NETO GRAVADO

		$object_writer = PHPExcel_IOFactory::createWriter($object, 'Excel5');
		$file_name = 'Libro Iva Compras '.$_POST["desde"].' a '.$_POST["hasta"].'.xls';
  ...
  <main>
    <div class="container-fluid mt-5">
      ...
      <div class="card text-center">
        <h3 class="card-header primary-color white-text">Descargar Libro Iva Compras a Excel</h3>
